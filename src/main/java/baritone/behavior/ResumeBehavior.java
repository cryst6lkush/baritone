...
public class ResumeBehavior extends Behavior implements Helper {
    ...
    private static final Set<String> TASK_COMMANDS = new HashSet<>(Arrays.asList(
            ...
            "thisway", "forward", "axis", "highway", "surface", "top", "litematica", "schematica", "build"
    ));
    ...
    public void onTick(TickEvent event) {
        ...
        this.tickCounter++;
        if (ctx.player() != null) {
            this.playerWasDead = !ctx.player().isEntityAlive();
        }
    ...
    private String currentServerId() {
        if (ctx.minecraft().isSingleplayer() && ctx.minecraft().getIntegratedServer() != null) {
            // the world folder name is the closest 1.12.2 equivalent of the 1.19 world path identity
            return "singleplayer:" + ctx.minecraft().getIntegratedServer().getFolderName();
        }
        ServerData server = ctx.minecraft().getCurrentServerData();
        return server == null ? null : server.serverIP;
    }
